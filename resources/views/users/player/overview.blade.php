                                    ...
                                    @foreach ( $village->buildings->on as $key => $building )
                                        @php
                                            ...
                                            if ( in_array( $loop->iteration, [ 1, 4, 7, 10, 13, 16 ] ) )
                                                $class .= " ps-sm-2 pe-sm-1 ps-xl-2 pe-xl-1";
                                            elseif ( in_array( $loop->iteration, [ 3, 6, 9, 12, 15, 18 ] ) )
                                                $class .= " ps-sm-1 pe-sm-2 ps-xl-1 pe-xl-2";
                                            else
                                                $class .= " px-sm-1 px-xl-1";

                                            $class .= ( $loop->iteration % 2 == 0 )
                                                      ? " ps-1 pe-2 ps-md-1 pe-md-2"
                                                      : " ps-2 pe-1 ps-md-2 pe-md-1";

                                            $png = Helper::getLevelImage( $key, $building->level );
                                            $img = "{$key}{$png}.png";
                                        @endphp

                                        <div class="col-6 col-sm-4 col-md-6 col-xl-4 my-1 {{ $class }}" title="{{ $building->name }}" >
                                            <a class="text-decoration-none text-dark" href="{{ route( "village.{$key}", [ "village" => $village ]) }}" >
                                                <div class="small-box h-100 d-flex flex-column justify-content-center border border-dark-subtle border-opacity-10 rounded" >
                                                    <img class="mx-auto w-100" style="max-width:140px!important" src="{{ asset( "assets/graphic/buildings/resized/{$img}" ) }}" alt="{{ $building->name }}" >
